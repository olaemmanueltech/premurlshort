...
<div class="card rounded-4 shadow-sm">
    <div class="card-body">
        <form method="post" action="<?php echo route('admin.users.update', [$user->id]) ?>" enctype="multipart/form-data">
            ...
            <?php
                $userExtra = is_string($user->extra ?? '') ? json_decode($user->extra, true) : ($user->extra ?? []);
                if (!is_array($userExtra)) $userExtra = [];
            ?>
            <div class="form-group mb-4">
                <label for="bio" class="form-label fw-bold"><?php ee('Author Bio') ?></label>
                <textarea class="form-control p-2" name="bio" id="bio" rows="4" placeholder="<?php ee('Short bio shown on blog posts...') ?>"><?php echo htmlspecialchars($userExtra['bio'] ?? '') ?></textarea>
